...
import { Case, APPLICATION_CRITERIA, ApplicationCriterion } from '@/types/case';
...
interface CaseApplicationDialogProps {
  caseItem: Case;
  open: boolean;
  onClose: () => void;
...
export function CaseApplicationDialog({
  caseItem,
  open,
  onClose,
  onStartResearch,
}: CaseApplicationDialogProps) {
  const { updateApplicationData, completeApplication } = useCases();

  const [explanation, setExplanation] = useState(caseItem.applicationData?.explanation || '');
  const [criteria, setCriteria] = useState<ApplicationCriterion[]>(
    caseItem.applicationData?.criteria || APPLICATION_CRITERIA.map((c) => ({
      id: c.id,
      name: c.name,
      label: c.label,
      isMet: false,
  ...
  useEffect(() => {
    setExplanation(caseItem.applicationData?.explanation || '');
    setCriteria(
      caseItem.applicationData?.criteria || APPLICATION_CRITERIA.map((c) => ({
        id: c.id,
        name: c.name,
  ...
  }, [caseItem.id, caseItem.applicationData]);
  ...
  const handleSave = () => {
    updateApplicationData(caseItem.id, {
      explanation,
      criteria,
    });
    onClose();
  ...
  const handleStartResearch = () => {
    ...
    completeApplication(caseItem.id);
    onStartResearch();
  };
