...
export function FolderAddSignalsDialog({
  folder,
  open,
  onClose,
}: FolderAddSignalsDialogProps) {
  ...
  const handleAddSignals = () => {
    if (selectedSignalIds.length > 0) {
      addSignalsToFolder(selectedSignalIds, folder.id);
      setSelectedSignalIds([]);
      setSearchQuery('');
      onClose();
    }
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Add Signals to Folder</DialogTitle>
          <DialogDescription>
            Select signals to add to: {folder.name}
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {availableSignals.length === 0
                  ? 'All signals are already in this folder'
