...
export function FolderDetailHeader({
  ...
  const { getSignalCountForFolder, updateFolder, assignFolderOwner, unassignFolderOwner } = useFolders();
  ...
  // Sync values when folder changes
  useEffect(() => {
  ...
  const saveName = async () => {
    if (nameValue.trim() && nameValue !== folder.name) {
      await updateFolder(folder.id, { name: nameValue.trim() });
    } else {
      setNameValue(folder.name);
    }
  ...
  const saveDescription = async () => {
    if (descriptionValue !== folder.description) {
      await updateFolder(folder.id, { description: descriptionValue });
    }
  ...
  const handleOwnerChange = async (userId: string | null) => {
    if (userId === null) {
      await unassignFolderOwner(folder.id);
    } else {
      const user = users.find(u => u.id === userId);
      if (user) {
        await assignFolderOwner(folder.id, userId, getUserFullName(user));
      }
    }
  };
                ...
                {FOLDER_COLORS.map((color) => (
                  <button
                    ...
                    className={cn(
                      ...
                      folder.color === color.value && 'ring-2 ring-offset-2 ring-primary'
                    )}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start">
                  <DropdownMenuItem
                    ...
                    className={cn(!folder.ownerId && 'bg-muted')}
                  >
                  ...
                  {users.map((user) => (
                    <DropdownMenuItem
                      ...
                      className={cn(folder.ownerId === user.id && 'bg-muted')}
                    >
