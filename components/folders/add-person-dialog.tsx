...
export function AddPersonDialog({
  ...
  const { addPersonToFolder } = useFolders();
  ...
  // Get existing person IDs in the folder
  const existingPersonIds = useMemo(() => {
    return new Set((folder.peopleInvolved || []).map((p) => p.id));
  }, [folder.peopleInvolved]);

  // Filter people that are not already in the folder
  ...
  const handleNewPersonCreated = (person: Person) => {
    // Add the newly created person to the folder
    addPersonToFolder(folder.id, person);
    setFormDialogOpen(false);
  };

  const handleBrpPersonSelected = (person: Person) => {
    // Add the person from BRP to the folder
    addPersonToFolder(folder.id, person);
      ...
      <Dialog open={open} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent className="w-[80vw] max-w-none sm:max-w-none max-h-[80vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Search for an existing person or add a new person to: {folder.name}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-muted-foreground">
                  {availablePeople.length === 0
                    ? 'All people have already been added to this folder'
