...
import { Folder, LetterItem, LETTER_TEMPLATES } from '@/types/folder';
...
export function FolderLetters({ folder }: FolderLettersProps) {
  const { addLetter, removeLetter, updateLetter } = useFolders();
  ...
  const letters = useMemo(() => folder.letters || [], [folder.letters]);
  ...
  const handleAddLetter = async (data: { name: string; template: string }) => {
    const newLetter = await addLetter(folder.id, {
      name: data.name,
  ...
  const handleDescriptionChange = (letterId: string, description: string) => {
    updateLetter(folder.id, letterId, { description });
  };

  const handleSaveFieldData = (fieldData: Record<string, string | boolean>) => {
    if (selectedLetter) {
      updateLetter(folder.id, selectedLetter.id, { fieldData });
    }
  };
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() => removeLetter(folder.id, letter.id)}
                            >
