...
export function registerCaseTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ status, limit }) => {
      const response = await apiRequest<Folder[]>({ method: 'GET', path: '/api/folders' });
  ...
  server.tool(
    ...
    {},
    async () => {
      const foldersResponse = await apiRequest<Folder[]>({ method: 'GET', path: '/api/folders' });
      const statsResponse = await apiRequest({ method: 'GET', path: '/api/folders/stats' });
